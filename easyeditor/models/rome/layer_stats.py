...
def layer_stats(
    ...
    def get_ds():
        ...
        raw_ds = Dataset.from_file(os.environ.get(
            'LAYER_STATS_DATASET',
            './data/stats/wikipedia-train.arrow'
        ))
        raw_ds = {'train': raw_ds}
        # raw_ds = load_dataset(
        #     ds_name,
    ...
    batch_size = 100 #100  # Examine this many dataset texts at once
    if hasattr(model.config, 'n_positions'):
    ...
    file_extension = f"{layer_name}_{precision}_{'-'.join(sorted(to_collect))}{size_suffix}.npz"

...
def test_prediction_acc_LLM_judge(model, tok, hparams, prompts, targets, device, locality=False):
    ...
    for prompt, target in zip(prompts, targets):
        ...
        # end_time = time.time()   # 修改了这里
        # inference_time = end_time-start_time   # 修改了这里
        # with open("base_inference_time_use_cache.txt", "a") as f:    # 修改了这里
        ...
        if hasattr(hparams, 'api_key') and hparams.api_key:
            # T5 get hidden
            # LLM_Score = llm_judge(prompt, target, gen_content.strip().split("\n")[0], hparams.api_key)
            LLM_Score = 0.0
            LLM_Contain_Score = float(any(possible_answer.lower() in gen_content.lower() for possible_answer in ([target] if isinstance(target, str) else target)))
            # T5 get hidden
            scores.append(LLM_Score)
            contain_scores.append(LLM_Contain_Score)
        contents.append(gen_content)
    return scores, contain_scores, contents
        # else:

...
class ActivationAnalyzer:
    ...
    def __init__(self, 
        ...
        Args:
            ...
            device: 计算设备，"auto" 自动选择，"cuda" 强制GPU，"cpu" 强制CPU
            use_fp16: 矩阵乘时是否使用 float16 加速（结果仍以 float32 返回）
        """
    ...
    def compute_perturbation(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            (delta_h, C, delta_h_norms) 元组，结果均在 CPU 上
        """
        ...
        # 搬到计算设备
        h_o = self._to_device(self.h_orig)   # [n, d]
        h_e = self._to_device(self.h_edit)   # [n, d]
        U_o = self._to_device(self.U_o)      # [d, d]

        # 计算扰动（float32，保持精度）
        delta_h = h_e - h_o                                        # [n, d]
        self.delta_h_norms = torch.linalg.norm(delta_h, dim=1).cpu()  # [n]

        # 矩阵乘：可选 float16 加速（误差通常 < 0.01%，对 R 值无影响）
        if self.use_fp16 and dev.type == "cuda":
            # float16 在 GPU 上约快 2x，完成后转回 float32
            C = (U_o.half().T @ delta_h.half().T).float()          # [d, n]
        else:
            C = U_o.T @ delta_h.T                                  # [d, n]

        # 结果拉回 CPU，后续 numpy 操作不依赖 GPU
        self.delta_h = delta_h.cpu()
        self.C       = C.cpu()

        # 显式释放 GPU 显存
        if dev.type == "cuda":
            del h_o, h_e, U_o, delta_h, C
            torch.cuda.empty_cache()

        print(f"扰动计算完成 (device={dev}):")
    ...
    def compute_R_values(self, epsilon: float = 1e-12) -> np.ndarray:
        ...
        sigma_cpu = self.sigma_original.cpu()
        d_actual  = min(d, self.C.shape[0])  # 防止维度越界

        # ✅ 核心优化：全向量化，无 Python 循环
        # 对 C 的每一行取 mean(|c_k|)，shape: [d_actual]
        mean_abs_C = torch.mean(torch.abs(self.C[:d_actual]), dim=1)

        # 构建 R 张量，先统一计算，再对极小奇异值做掩码覆盖
        sigma_slice = sigma_cpu[:d_actual]
        valid_mask  = sigma_slice >= epsilon

        R_tensor = torch.where(
            valid_mask,
            (sqrt_n * mean_abs_C) / torch.clamp(sigma_slice, min=epsilon),
            torch.full_like(mean_abs_C, 1e20)
        )

        self.R_values = R_tensor.numpy()
        self.k_index  = np.arange(1, len(self.R_values) + 1)  # 主成分索引，从1开始
        
        print(f"R值计算完成: 共{len(self.R_values)}个值")
        print(f"  R_min: {self.R_values.min():.6f}")
        print(f"  R_max: {self.R_values.max():.6f}")
        print(f"  R_mean: {self.R_values.mean():.6f}")
    ...
    def save_R_values(self, filename: str = "R_values.npz") -> str:
        ...
        # 修复原代码 bug：self.output → self.output_dir
        file_path = os.path.join(self.output_dir, filename)
    ...
    def plot_analysis(self, 
        ...
        stats_text = (
            f"Statistics:\n"
            f"Mean R: {np.mean(self.R_values):.3f}\n"
            f"Max R: {np.max(self.R_values):.3f}\n"
            f"R>1: {np.sum(self.R_values > 1.0)} ({np.mean(self.R_values > 1.0)*100:.1f}%)"
        )
        ...
        svd_text = (
            f"Singular Values:\n"
            f"Max σ: {sigma_np[0]:.3e}\n"
            f"Min σ: {sigma_np[d-1]:.3e}\n"
            f"Condition: {sigma_np[0]/sigma_np[d-1]:.3e}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--edit_pt")
    parser.add_argument("--orig_pt")
    parser.add_argument("--output_dir")
    parser.add_argument("--fig_name")
    parser.add_argument("--device", default="auto",
                        help="计算设备: auto / cuda / cpu")
    parser.add_argument("--open_fp16", action="store_true",
                        help="开启 float16 加速（默认禁用）")
    ...
    analyzer = ActivationAnalyzer(
        ...
        device=args.device,
        use_fp16=args.open_fp16,
    )

    analyzer.load_json_data(args.json_path)
    analyzer.load_npz_data(args.npz_path)
    analyzer.load_pt_data(
        edit_path=args.edit_pt,
        orig_path=args.orig_pt
    )
    ...
    analyzer.save_results(stats, args.fig_name.replace(".png", ".json"))

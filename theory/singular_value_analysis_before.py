...
def analyze_one_layer(analyzer, orig_path, edit_path, layer_name, module):
    ...
    # H_o = load_hidden_state(orig_path, module)
    # H_e = load_hidden_state(edit_path, module)
    H_orig = load_hidden_state(edit_path, module)
    H_edit = load_hidden_state(edit_path, module)

    H_orig = H_orig.T.float()
    H_edit = H_edit.T.float()

    mean_vec = H_orig.mean(dim=1, keepdim=True)
...
def main():
    ...
    for step in args.edit_steps:
        ...
        for L in args.layers:
            ...
            # save per-layer
            layer_save_path = save_dir / f"{step}_layer_{L}.json"

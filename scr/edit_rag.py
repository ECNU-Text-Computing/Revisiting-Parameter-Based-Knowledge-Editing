...
class RetrieveTool():
    ...
    def get_sent_embeddings(self, memory, BSZ=1):
        ...
        # 记录 embedding_time，例如写入文件或累加到一个列表中
        with open("scr_embed_time.txt", "a") as f:
            f.write(f"Embedding time for {len(memory)} sentences: {embedding_time:.4f} seconds\n")
        return  # 注意: 此函数没有返回值，但时间被记录
    ...
    def retrieve(self, query, memory, k):
        ...
        end_time = time.time()
        single_retrieve_time = end_time - start_time
        # 记录 single_retrieve_time
        with open("scr_single_retrieve_time.txt", "a") as f:
            f.write(f"Single retrieve time: {single_retrieve_time:.4f} seconds\n")
...
class SummaryTool():
    ...
    def summary(self, question, retrieved_facts, summary_prompt, max_new_tokens=50):
        ...
        with open("scr_summary_time.txt", "a") as f:
            f.write(f"Summary time: {summary_time:.4f} seconds\n")
    ...
    def single_edit(self, prompt, target_new, knowledge=None, locality=False, vanilla_genration=True,
        ...
        if eval_metric == "contain":
            ...
            if knowledge is None:
                ...
                # 记录 inference_time
                with open("scr_inference_time.txt", "a") as f:
                ...
                inference_time = end_time - start_time
                with open("scr_inference_time.txt", "a") as f:
                    f.write(f"Inference time (with knowledge): {inference_time:.4f} seconds\n")
    ...
    if edit_scene=="sequential":
        ...
        for i, request in enumerate(tqdm(requests, total=len(requests))):
            ...
            if "prompt" in request:
                ...
                with open("/fs-computility/ai-shen/songxin/EasyEdit/time_plots/base_inference_time_use_cache.txt", "a") as f:    # 修改了这里
                    f.write(f"Inference time: {inference_time:.4f} seconds\n")  
        ...
        # 可以将total_time记录到一个全局变量或日志中
        print(f"Total execution time for {len(requests)} requests: {total_time:.4f} seconds")
    else:
        # all_metrics = []
        # 得到所有memory的embedding表示
        ...
        for i, request in enumerate(tqdm(requests, total=len(requests))):
            #print(memory[i])
            #retrieve_tool.get_sent_embeddings(memory[i], BSZ=1)
            ret = {}
            ret = request
            ret["post"]={}
...
if __name__ == "__main__":
    ...
    if args.memory_path:
        ...
        retriever_size_mb = (retriever_params * 4) / (1024**2)  # 假设float32, 单位MB
        with open("scr_memory_footprint.txt", "a") as f:
            f.write(f"Retriever model parameters: {retriever_params}\n")
            f.write(f"Estimated retriever model size (float32): {retriever_size_mb:.2f} MB\n")
        # 估算 memory 大小
        if memory is not None:
            memory_size_chars = sum(len(s) for s in memory)
            memory_size_mb = (memory_size_chars * 1) / (1024**2)  # 粗略估算: 1字节/字符
            with open("scr_memory_footprint.txt", "a") as f:
                f.write(f"Memory (corpus) size in characters: {memory_size_chars}\n")
                f.write(f"Estimated memory storage: {memory_size_mb:.2f} MB\n")

    # 加载数据
    # data = load_data(args.data_path, eval_metric=args.eval_metric)
    data = load_data(args.data_path, datatype = 'counterfact')

    # 构建请求
    if args.ds_size is None:
        requests = _prepare_requests(prompts=data["prompts"], target_new=data["target_new"])
    else:
        requests = _prepare_requests(prompts=data["prompts"], target_new=data["target_new"])[
